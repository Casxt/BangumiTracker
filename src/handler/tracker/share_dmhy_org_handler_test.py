from proto_py.base import resources_pb2
...
def test__extra_episode_index():
    ...
    string = "[231115][不死不幸]TVアニメ『アンデッドアンラック』EDテーマ「know me...」／八木海莉[320K]"
    assert ShareDMHYTrackerHandler._extra_episode_index(string) == ""


def test__split_title():
    string = "【極影字幕·毀片黨】我們的雨色協議 第10集 BIG5 (AAC_AVC 720p) 『アンデットアンラック』OP2 & ED2「ラブコール」"
    assert ShareDMHYTrackerHandler._split_title(string) == [
        '極影字幕·毀片黨', '我們的雨色協議', '第10集', 'BIG5', "AAC", 'AVC', '720p', "アンデットアンラック", "OP2", "&", "ED2", "ラブコール"]


def test__collecting_tag():
    # [Hi-Res][240120][不死不幸]TVアニメ『アンデットアンラック』OP2 & ED2「ラブコール」シユイ 「この愛に敵うもんはない」OKAMOTO'S[96kHz/24bit][FLAC]
    string = "[喵萌奶茶屋&LoliHouse] 福星小子 2022年版 / Urusei Yatsura 2022 - 32 [WebRip 1080p HEVC-10bit AAC][简繁日内封字幕] 320K FLAC"
    ...
    assert tag_strs == ["喵萌奶茶屋&LoliHouse", "WEBRIP", "1080P",
                        "HEVC", "10BIT", "AAC", "简繁日内封字幕", "320K", "FLAC"]
...
def test_filter_resources():
    resources = [
        resources_pb2.ExternalResource(
            share_dmhy_org=resources_pb2.SHARE_DMHY_ORG(title="some bangumiⅡ")),
        resources_pb2.ExternalResource(
            share_dmhy_org=resources_pb2.SHARE_DMHY_ORG(title="some bangumi")),
    ]
    new_resources = ShareDMHYTrackerHandler.filter_resources(
        resources, include_regex=r'Ⅱ|第二季|[sS]2|2期')
    new_resources = tuple(new_resources)
    assert len(new_resources) == 1
    assert new_resources[0].share_dmhy_org.title == "some bangumiⅡ"
    new_resources = ShareDMHYTrackerHandler.filter_resources(
        resources, exclude_regex=r'Ⅱ|第二季|[sS]2|2期')
    new_resources = tuple(new_resources)
    assert len(new_resources) == 1
    assert new_resources[0].share_dmhy_org.title == "some bangumi"

    new_resources = ShareDMHYTrackerHandler.filter_resources(
        resources, include_regex=r'Ⅱ|第二季|[sS]2|2期', exclude_regex=r'Ⅱ|第二季|[sS]2|2期')
    assert len(tuple(new_resources)) == 0

    resources = [
        resources_pb2.ExternalResource(
            share_dmhy_org=resources_pb2.SHARE_DMHY_ORG(title="[Skymoon-Raws] 無職轉生，到了異世界就拿出真本事 第2季 / Mushoku Tensei 2nd Season - 09 [ViuTV][WEB-RIP][1080p][AVC AAC][CHT][SRT][MKV](先行版本) IPFS服務器種"
)),
    ]
    new_resources = ShareDMHYTrackerHandler.filter_resources(
        resources, exclude_regex=r'(Ⅱ|第2季|第二季|S2|s2|2期)')
    assert len(tuple(new_resources)) == 0